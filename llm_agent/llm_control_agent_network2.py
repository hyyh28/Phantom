...
class LLMDecisionStrategy(DecisionStrategy):
    ...
    def _generate_system_prompt(self, agent_profile: Dict[str, Any], action_space: Any) -> str:
        ...
        - The valid range is from {min_action} to {max_action}
    ...
    def _parse_and_validate_action(self, response: str, action_space: Any) -> float:
        ...
        try:
            ...
            if hasattr(action_space, "n"):  # Discrete
                ...
                min_action = action_space.low[0]
                max_action = action_space.high[0]
            action_value = max(min_action, min(action_value, max_action))
...
def run_llm_agent_simulation(env_class, model="gpt-4o", num_episodes=1, num_steps=100,
    ...
    for episode in range(num_episodes):
        ...
        for step in range(num_steps):
            ...
            for agent in agents_name:
                total_reward[agent] += reward.get(agent, 0)
        ...
        # Print final state of retailer agents
        for retailer_id in ['RETAILER1', 'RETAILER2', 'RETAILER3', 'RETAILER4']:
            retailer_agent = env.agents[retailer_id]
            print(f"{retailer_id} - Final inventory: {retailer_agent.stock}")
            print(f"{retailer_id} - Last step sales: {retailer_agent.sales}")
            print(f"{retailer_id} - Last step missed sales: {retailer_agent.missed}")
...
# network2 environment test
# Usage example
if __name__ == "__main__":
    from network2 import Network2Env

    agent_names = ['RETAILER1', 'RETAILER2', 'RETAILER3', 'RETAILER4']
    agent_roles = ['Retailer'] * 4
    agents_personality = ['Self-Interested'] * 4
    # Run simulation with default parameters
    run_llm_agent_simulation(
        env_class=Network2Env,
        model="deepseek",
        num_episodes=1,
        num_steps=50,
        agents_role=agent_roles,
        agents_name=agent_names,
